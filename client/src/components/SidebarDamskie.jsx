  ...
  return (
    <div className="relative inline-block">
      <button
        ...
        className="text-black hover:text-white px-3 py-2 rounded transition-colors duration-300">Damskie</button>
          ...
          <ul className="list-disc list-inside p-4 space-y-2" >
                                {kategorielista.map((kategoria) => (
                                    <li key={kategoria._id} className="text-xl font-bold text-center text-gray-800">
                                        <Link to={`/CattegoryProductsView/Damskie/${kategoria.name}`}>{kategoria.name}</Link>
                                    </li>
                                ))}
                            </ul>
